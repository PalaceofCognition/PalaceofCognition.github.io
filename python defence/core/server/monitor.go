...
func Monitor() {
  ...
  fmt.Print(utils.PurpleText(" ╚═════╝ ╚══════╝ ╚═════╝      ╚═════╝  ╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═╝╚═════╝ \n"))
	...
	PrintMutex.Lock()
	fmt.Print("[ " + utils.GreenText("ბრძანება") + " ]: \033[s")
	PrintMutex.Unlock()
	for {
		PrintMutex.Lock()
...
func printStats() {
	...
	if err != nil {
		proxy.CpuUsage = "ERR"
		fmt.Println("[" + utils.GreenText("✪") + "] ▎ " + utils.GreenText("Cpu გამოყენება") + " ▎ ► ▛ " + utils.GreenText(err.Error()) + " ▜")
	} else if len(result) > 0 {
		proxy.CpuUsage = fmt.Sprintf("%.2f", result[0])
		fmt.Println("[" + utils.GreenText("✪") + "] ▎ " + utils.GreenText("Cpu გამოყენება") + " ▎ ► ▛ " + utils.GreenText(proxy.CpuUsage) + " ▜")
	} else {
		proxy.CpuUsage = "ERR_S0"
		fmt.Println("[" + utils.GreenText("✪") + "] ▎ " + utils.GreenText("Cpu გამოყენება") + " ▎ ► ▛ " + utils.GreenText("100.00 ( Speculated )") + " ▜")
	}
	...
	if domainData.Stage == 0 {
		if proxy.WatchedDomain != "" {
			fmt.Println("[" + utils.GreenText("!") + "] [ " + utils.GreenText("Domain \""+proxy.WatchedDomain+"\" Not Found") + " ]")
			fmt.Println("")
		}
		fmt.Println("[" + utils.GreenText("Available Domains") + "]")
		counter := 0
		for _, dName := range domains.Domains {
			if counter < proxy.MaxLogLength {
				fmt.Println("[" + utils.GreenText("+") + "] [ " + utils.GreenText(dName) + " ]")
				counter++
			}
		}
	} else {
	  fmt.Println("" + utils.CyanText("") + "  " + utils.CyanText("꧁GeoGuard Services꧂") )
    fmt.Println("" + utils.GreenText("") + "  " + utils.GreenText("") )
		fmt.Println("" + utils.YellowText("☢") + " ▛ " + utils.GreenText("დომენი") + " ▜  ► ▛ " + utils.GreenText(proxy.WatchedDomain) + " ▜")
		fmt.Println("" + utils.YellowText("☢") + " [ " + utils.GreenText("ფაზა") + " ]  ► [ " + utils.GreenText(fmt.Sprint(domainData.Stage)) + " ]")
		fmt.Println("" + utils.YellowText("☢") + " [ " + utils.GreenText("ფაზის დაბლოკვა") + " ] ► [ " + utils.GreenText(fmt.Sprint(domainData.StageManuallySet)) + " ]")
		fmt.Println("")
		fmt.Println("" + utils.YellowText("☢") + " [ " + utils.GreenText("სულ შემოსულია") + " ]  ► [ " + utils.GreenText(fmt.Sprint(domainData.RequestsPerSecond)+" r/s") + " ]")
		fmt.Println("" + utils.YellowText("☢") + " [ " + utils.GreenText("გვერდის ავლა") + " ]  ► [ " + utils.GreenText(fmt.Sprint(domainData.RequestsBypassedPerSecond)+" r/s") + " ]")

		fmt.Println("")
		fmt.Println("⇘ " + utils.GreenText("ლოგების სივრცე") + " ⇙")

		for _, log := range domainData.LastLogs {
			if len(log)+4 > proxy.TWidth {
				fmt.Println("[" + utils.GreenText("+") + "] " + log[:len(log)-(len(log)+4-proxy.TWidth)] + " ...\033[0m")
			} else {
				fmt.Println("[" + utils.GreenText("+") + "] " + log)
			}
		}
	}
...
func commands() {
	...
	for {
		if scanner.Scan() {
			...
			PrintMutex.Lock()
			fmt.Println("\033[" + fmt.Sprint(12+proxy.MaxLogLength) + ";1H")
			fmt.Print("\033[K[ " + utils.GreenText("ბრძანება") + " ]: \033[s")
			...
			case "domain":
				...
				screen.Clear()
				screen.MoveTopLeft()
				fmt.Println("[ " + utils.GreenText("დაელოდეთ") + " ] ...")
				fmt.Println("\033[" + fmt.Sprint(12+proxy.MaxLogLength) + ";1H")
				fmt.Print("[ " + utils.GreenText("ბრძანება") + " ]: \033[s")
			case "add":
				screen.Clear()
			...
			case "rtlogs":
				...
				if proxy.RealTimeLogs {
					...
					fmt.Println("[ " + utils.GreenText("Turning Real Time Logs Off") + " ] ...")
				} else {
					proxy.RealTimeLogs = true
					fmt.Println("[ " + utils.GreenText("Turning Real Time Logs On") + " ] ...")
				}
				fmt.Println("\033[" + fmt.Sprint(12+proxy.MaxLogLength) + ";1H")
				fmt.Print("[ " + utils.GreenText("ბრძანება") + " ]: \033[s")
			case "delcache":
				screen.Clear()
				screen.MoveTopLeft()
				fmt.Println("[ " + utils.GreenText("Clearing Cache For "+proxy.WatchedDomain) + " ] ...")
				clearCache()
				fmt.Println("\033[" + fmt.Sprint(12+proxy.MaxLogLength) + ";1H")
				fmt.Print("[ " + utils.GreenText("ბრძანება") + " ]: \033[s")
			default:
...
func reloadConfig() {
	...
	for i, domain := range domains.Config.Domains {
		...
		for _, fwRule := range domains.Config.Domains[i].FirewallRules {
			...
			if err != nil {
				panic("[ " + utils.GreenText("!") + " ] [ Error Loading Custom Firewall Rules: " + utils.GreenText(err.Error()) + " ]")
			}
		...
		for _, caRule := range domains.Config.Domains[i].CacheRules {
			...
			if err != nil {
				panic("[ " + utils.GreenText("!") + " ] [ Error Loading Custom Cache Rules: " + utils.GreenText(err.Error()) + " ]")
			}
		...
		if !proxy.Cloudflare {
			...
			if certErr != nil {
				panic("[ " + utils.GreenText("!") + " ] [ " + utils.GreenText("Error Loading Certificates: "+certErr.Error()) + " ]")
			}
		}
