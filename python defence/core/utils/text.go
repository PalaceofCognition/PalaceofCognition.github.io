...
func AddLogs(entry string, domainName string) domains.DomainData {
		...
		if proxy.RealTimeLogs {
			...
			for i, log := range domainData.LastLogs {
				if len(log)+4 > proxy.TWidth {
					fmt.Print("\033[" + fmt.Sprint(11+i) + ";1H\033[K[" + GreenText("!") + "] " + log[:len(log)-(len(log)+4-proxy.TWidth)] + " ...\033[0m\n")
				} else {
					fmt.Print("\033[" + fmt.Sprint(11+i) + ";1H\033[K[" + GreenText("!") + "] " + log + "\n")
				}
			}
			MoveInputLine()
			PrintMutex.Unlock()
	...
	if domainName == proxy.WatchedDomain && proxy.RealTimeLogs {
		...
		if len(entry)+4 > proxy.TWidth {
			fmt.Print("\033[" + fmt.Sprint((10 + len(domainData.LastLogs))) + ";1H\033[K[" + GreenText("-") + "] " + entry[:len(entry)-(len(entry)+4-proxy.TWidth)] + " ...\033[0m\n")
		} else {
			fmt.Print("\033[" + fmt.Sprint((10 + len(domainData.LastLogs))) + ";1H\033[K[" + GreenText("-") + "] " + entry + "\n")
		}
		MoveInputLine()
		PrintMutex.Unlock()
	}
...
func MoveInputLine() {
	...
	fmt.Print("[ " + YellowText("ბრძანება") + " ]: \033[u\033[s")
}

func RedText(input string) string {
	return "\033[31m" + input + "\033[0m"
}
func CyanText(input string) string {
...
func YellowText(input string) string {
	return "\033[33m" + input + "\033[0m"
}

func PurpleText(input string) string {
